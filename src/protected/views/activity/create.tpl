		...
		<div class='span6'>
			<div class='activity_form' id='activity_form'>
					...
					<div class='line'>
						...
						<img id='upload_post' class='col' width='91px' height='121px' src='{{$static}}/img/upload_img.png'/>
						<div class='col upload_tips'>
							<p>从电脑中选择有吸引力的图片作为活动海报图片</p>
							<p>你可以上传JPG、JPEG、GIF、PNG或BMP文件。文件小于5M</p>
                            <input type='button' class='upload_button' onclick="document.getElementById('ActivityForm_post_url').click()" value='选择图片'/>                            
						</div> 
                        ...
                        <div class='input_error'>{{CHtml::error($form,'post_url')}}</div>
						<div class='clear'></div>                      
					</div>
					<div class='line'>
					...
					<div class='line'>
						...
						<select id="s_province" name="s_province" class="address_select">
							<option selected="true">选择省份</option>
						</select>&nbsp;&nbsp;
						<select id="s_city" name="s_city" class="address_select">
                        	<option selected="true">选择城市</option>
                        </select>&nbsp;&nbsp;
					...
					<hr/>
					<div class='line'>
                        <input type="submit" value="发布" name="yt0" class='pub_button col'>&nbsp;&nbsp;&nbsp;&nbsp;<p class='col ml20'><a href='/activity/manage'>取消</a>(将返回已发布活动的列表)<p>
					</div>
				{{CHtml::endForm()}}
			</div>
		</div>
		...
		if($('#start_time_input').val()==''){
			...
			if($('#end_time_input').val()==''){
				...
				if(checkEndTime()==false){
					$('#end_time_error').html('活动结束时间不能早于活动开始时间');	
					return false;
				}else{
					$('#end_time_error').html('');
