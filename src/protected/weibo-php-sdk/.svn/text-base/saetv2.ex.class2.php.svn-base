...
class SaeTOAuthV2 {
	...
	function getAccessToken( $type = 'code', $keys ) {
		...
		if ( is_array($token) && !isset($token['error']) ) {
			...
			//commented by zhoulei 2012-10-27 $this->refresh_token = $token['refresh_token'];
		} else {
...
class SaeTClientV2
{
	 ...
	 *
	 * 将一个或多个短链接还原成原始的长链接。
	 * <br />对应API：{@link http://open.weibo.com/wiki/2/short_url/expand}
	 * 
	 * @param string $url_short 需要还原的短链接，需要URLencoded，最多不超过20个 。 。
	 * @return array
	 */
	function url_short_expand( $url )
	{
		$params = array();
  ...
  /** added by zhoulei 
	 * 根据微博id号跳转微博详细页面
	 *
	 * 对应API：{@link http://open.weibo.com/wiki/2/statuses/go }
	 *
	 * @param string $wid 用户的微博ID 。
	 * @param string $id 根据ID跳转到单条微博页 。
	 * @return array
	 */
    function gocid( $wid,$id )
	{
		$params=array();
		$params['uid'] = $wid;
		$params['id'] = $id;
		
		return $this->oauth->get('statuses/go', $params);
	}
	
	
   /** added by zhoulei 
	 * 根据微博id号获取微博的评论数和转发数信息
	 *
	 * 对应API：{@link http://open.weibo.com/wiki/2/statuses/count }
	 *
	 * @param string $ids 需要查询的微博ID，用半角逗号分隔，最多不超过100个。
	 * @return array
	 */
    function count_batch( $ids )
	{
		$params=array();
	 ...
	 *
	 * 按用户UID或昵称返回用户的等级信息
	 * <br />对应API：{@link http://open.weibo.com/wiki/2/users/show users/show_rank}
	 * 
	 * @access public
	 * @param int  $uid 用户UID。
	 * @return array
	 */
	function show_user_rank_by_id( $uid )
	{
		$params=array();
		if ( $uid !== NULL ) {
			$this->id_format($uid);
		...
		return $this->oauth->get('users/show_rank', $params );
	}
	 ...
	 * <br />对应API：{@link http://open.weibo.com/wiki/2/search/suggestions/atusers}
	 * 
	 * @access public
	 * @param int $q 搜索的关键字，必须做URLencoding。 。
	 * @param int $count 返回的记录条数，默认为10，粉丝最多1000，关注最多2000。 
	 * @param int $type  联想类型，0：关注、1：粉丝。 
	 * @param int $range  联想范围，0：只联想关注人、1：只联想关注人的备注、2：全部，默认为2。 
	 * @return array
	 */
	function sugges_at_user( $q, $count = 1000, $type = 1, $range=2)
	{
		$params = array();
		$params['q'] = $q;
		$params['count'] = $count;
